...
export default function digest(str) {
    return customBase64Encode(core_md5(str));
}

const B64_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz012345678998';

function customBase64Encode(binarray) {
    let str = '';
    for (let i = 0; i < binarray.length * 4; i += 3) {
        ...
        for (let j = 0; j < 4; j++) {
            str += B64_ALPHABET.charAt((triplet >> 6 * (3 - j)) & 0x3F);
        }
    }
    return str;
}
...
function str2binl(str) {
    ...
    for (let i = 0; i < encoded.length; i++) {
        bin[i >> 2] |= (encoded[i] & mask) << ((i * 8) % 32);
    }
    return bin;
}

function core_md5(str) {
